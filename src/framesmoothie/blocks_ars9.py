"""ARS9-based conditioned mixing block (mirror of RS9CondMixBlock).

Uses ARS9Layer (conjugate-pair complex internal state, real I/O) instead of RS9Layer.
...
class ARS9CondMixBlock(nn.Module):
    """
    Attention-free, ARS9-based conditioned mixing block.

    Structurally identical to ``RS9CondMixBlock`` but uses ARS9Layer
    (complex conjugate-pair internal state, real I/O) as the global mixer.
    ARS9 supports oscillatory dynamics that the purely real RS9 cannot.
    ...
    def __init__(
        ...
        super().__init__()
        self.ars9: ARS9Layer = ars9 if ars9 is not None else ARS9Layer(
            d_model=c_model,
    ...
    def forward(self, x: torch.Tensor, q: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        q: [B, K, Dq]
        """
        if (x.ndim - 2) != self.ars9.spatial_dims:
            raise ValueError(f"spatial_dims mismatch: x has {x.ndim-2}, ars9 has {self.ars9.spatial_dims}")
